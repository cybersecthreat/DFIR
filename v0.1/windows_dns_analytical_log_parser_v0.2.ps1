               ...
               ForEach ($dns_row in $($data -split "`r`n"))
                {
                   ...
                   if ($NextLineIsDescription -eq 1) {
                        ...
                        $DNSLogHashTable.add('DNS_DATA_TYPE', $DNS_DATA_TYPE)
                        $DNS_DATA_TYPE=$null
                        ...
                        while ($match.Success) {
                            ...
                            #}
                            if ($match.Groups['name'].Value -eq "PacketData" -and $DNSLogHashTable['DNS_DATA_TYPE'] -eq "RESPONSE_SUCCESS") {
                                
                                    [string]$PacketData_temp=$match.Groups['value'].Value.Substring(2);
                                    write-host $PacketData_temp.GetType();
                                    $DNSPacketData = [DNSPacket]::new();
                                    $PacketData_temp_ByteArray=Convert-HexStringToByteArray($PacketData_temp);
                                    $PacketData_temp=$null
                        ...
                        $DNSLogHashTable.add('DNS_DATETIME', $DNS_DATETIME)
                        $DNS_DATETIME=$null
                   } elseif ($dns_row -like "  Computer: *"){
                        $DNS_Computer=$dns_row.split(':',2)[1].trim()
                        $DNSLogHashTable.add('DNS_Computer', $DNS_Computer)
                        $DNS_Computer=$null
